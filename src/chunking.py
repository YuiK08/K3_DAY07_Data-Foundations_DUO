...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        # No more separators to try — character-level split
        ...
        current_group = ""
        for part in parts:
            candidate = (current_group + sep + part) if current_group else part
            if len(candidate) <= self.chunk_size:
                current_group = candidate
            else:
                # Flush current_group
                if current_group.strip():
                    if len(current_group) <= self.chunk_size:
                        results.append(current_group)
                    else:
                        results.extend(self._split(current_group, rest))
                # Start new group with current part
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        for name, chunker in strategies.items():
            ...
            count = len(chunks)
            avg_length = (
                sum(len(c) for c in chunks) / count if count > 0 else 0.0
            )
            result[name] = {
                "count": count,
